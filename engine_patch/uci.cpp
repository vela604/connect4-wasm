...
UCI::UCI(bool web_mode) : engine_(256), web_mode_(web_mode) {
...
void UCI::print_info(const SearchResult& result, const SearchStats& stats) {
    ...
    if (web_mode_) {
        // Clean, ANSI-free, single-line format the JS Web Worker parses
        // with a regex — analogous to a standard UCI "info" line, but
        // using this engine's own score/mate semantics (see engine.h):
        // score is a raw evaluation unit (not centipawns) and mate_in
        // is already signed (+N = win in N plies, -N = lose in N).
        long ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                       std::chrono::steady_clock::now() - go_start_).count();
        if (ms < 1) ms = 1;
        ...
        if (std::abs(result.score) >= MATE_THRESHOLD)
            std::cout << "mate " << result.mate_in;
        else
            std::cout << "cp " << result.score;
        std::cout << " nodes " << nodes
                   << " nps " << nps
                   << " time " << ms
                   << " ttpct " << std::fixed << std::setprecision(1) << pct_p
                   << " tthits " << tt_hits;
        if (result.best_move >= 0)
            std::cout << " currmove " << result.best_move;
        if (!result.pv.empty()) {
            std::cout << " pv";
...
void UCI::print_final_result(const SearchResult& result) {
    if (web_mode_) {
        // Interrupted search (superseded by a newer 'go' before depth 1
        // completed) — nothing meaningful to report, stay silent so the
        // JS side doesn't mistake it for bestmove none / a real result.
        if (result.depth == 0 && result.best_move < 0) return;

        std::cout << "bestmove "
                   << (result.best_move >= 0 ? std::to_string(result.best_move) : "none")
                   << "\n";
        std::cout.flush();
        return;
    }
...
//  Main loop
// ─────────────────────────────────────────────
void UCI::execute(const std::string& raw_line) {
